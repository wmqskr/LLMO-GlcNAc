...
def test_model(test_data, test_label):
    ...
    model = DebertaV2ForSequenceClassification.from_pretrained("../deberta-v3-base",
                                                        num_labels=1).to(device)
    
    # 加载保存的最佳模型
    model.load_state_dict(torch.load('../Result/6T101_deberta_model.pth', map_location='cuda:2'))
    model.eval()
