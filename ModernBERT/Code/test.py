...
def test_model(test_data, test_label):
    ...
    model = ModernBertForSequenceClassification.from_pretrained("../modernbert",
                                                        num_labels=1).to(device)
    
    # 加载保存的最佳模型
    model.load_state_dict(torch.load('../Result/16T101_modernbert_model.pth', map_location='cuda:2'))
    model.eval()
