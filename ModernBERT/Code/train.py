...
from transformers import AutoTokenizer, ModernBertForSequenceClassification, logging
import numpy as np
...
def train_and_evaluate_model_with_cv(data, labels):
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(input_ids)):
        ...
        model = ModernBertForSequenceClassification.from_pretrained("../modernbert", num_labels=num_labels).to(device)
    ...
    # torch.save(model_save, "../Result/16T101_modernbert_model.pth")
