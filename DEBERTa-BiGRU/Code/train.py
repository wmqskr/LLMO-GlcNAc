...
def train_and_evaluate_model_with_cv(data, labels):
    ...
    Args:
    - data (list of str): Protein sequences in FASTA format.
    - labels (list of int): Labels corresponding to each protein sequence.
    - model_name (str): Name or path to the pre-trained model.
    - num_labels (int): Number of labels for classification (default 1).
    - batch_size (int): Batch size for training and evaluation (default 32).
    - num_epochs (int): Number of epochs for training (default 4).
    
    Returns:
    - None: This function logs the results to a file and prints the final cross-validation metrics.
    """
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(input_ids)):
        ...
        # Initialize model
        model = DeBERTaBiGRUClassifier().to(device)
        # 提取deberta和gru的参数
        deberta_params = [p for p in model.deberta.parameters()]
        gru_params = [p for p in model.gru.parameters()]
        optimizer = optim.AdamW([
            {'params': deberta_params, 'lr': 2e-5},  # deberta的学习率
            {'params': gru_params, 'lr': 1e-3}   # gru的学习率
        ])
    ...
    # torch.save(model, "../Result/13T101_debertabigru_model.pth")
    # print("Model saved.")
